...
class PageParser(Parser):
    ...
    def get_question_url(self, info):
        """获取微博问答的url"""
        question_url = ''
        text = handle_garbled(info)
        if text.startswith(u'我回答了'):
            url = info.xpath('.//a/@href')
            if url and url[1].startswith('https://weibo.cn/sinaurl'):
                question_url = url[1]
        return question_url
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if (not self.filter) or is_original:
                ...
                weibo.question_url = self.get_question_url(info)  # 微博问答url
